...
\code{wait} und \code{notify} ermöglichen Implementierungen \textit{aktives warten} nutzen zu müssen: Die Methoden werden auf ein Objekt angewendet, das der aufrufende Thread zum Zeitpunkt des Aufrufs durch \code{synchronized} gesperrt hat.

\begin{tcolorbox}
    Der aufrufende Thread von \code{wait} / \code{notify} muss auch das Objekt über \code{synchronized} gesperrt haben, ansonsten wird eine \code{java.lang.IllegalMonitorStateException} geworfen\footnote{
...
Danach wird die Sperre des Objektes aufgehoben.
...
Die Sperre des Objektes wird aufgehoben, sobald die \code{synchronized}-Methode, in der \code{notify()} aufgerufen wurde, beendet ist.\\
