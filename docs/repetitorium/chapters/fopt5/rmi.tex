...
\begin{tcolorbox}
    Identität und Gleichheit von zwei \textbf{Stubs} (Rückgabewert von \code{Naming.lookup(...)}) führt zu \code{false}, wenn ein Stub für ein und denselben Namen angefordert wird, \code{equals()} hingegen zu \code{true}.\\

    $\rightarrow$ \textbf{Stub-Objekte} sind dann nicht ``identisch``, aber wenn sie auf dasselbe \textbf{RMI-Objekt} zeigen, werden sie als ``gleich`` betrachtet.
\end{tcolorbox}
...
\begin{tcolorbox}
    Methodenaufrufe auf ein RMI-Objekt finden parallel statt durch mehrere Clients, beim Zugriff auf gemeinsame Daten muss entsprechend synchronisiert werden,
\end{tcolorbox}