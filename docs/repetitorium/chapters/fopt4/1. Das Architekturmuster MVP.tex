...
\noindent
Mit der \ul{statischen Methode} \begin{center}\code{javafx.application.Platform.runLater(runnable: Runnable)}\end{center} lassen sich Aktionen am \textbf{JavaFX Application Thread} \textit{anmelden} bzw. in dessen \textit{Warteschlange} einreihen.\\
...
} (Aufruf der Methode \code{setDaemon(true)} auf ein Objekt der Klasse \code{Thread}, \ul{bevor} der Thread gestartet wurde).\\
